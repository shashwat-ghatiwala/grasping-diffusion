...
class Conv3DModel(nn.Module):
    ...
    def __init__(self, level_channels=[8, 16, 32]) -> None:
        ...
        # Layer 1
        self.conv1_1 = nn.Conv3d(in_channels=1, out_channels=l1, kernel_size=(3,3,3), padding=1)
    ...
    def forward(self, input):
        ...
        res = self.relu(self.conv1_1(input))
        ...
        res = self.relu(self.conv3_1(res))
        # res = self.relu(self.conv3_2(res))
        # res = self.relu(self.bn3(self.conv3_1(res)))
        res = self.pooling(res)
        # print("After 3rd layer: ", out.shape)

        out = self.relu(self.conv4_1(res))
        # res = self.relu(self.bn4(self.conv4_1(res)))
        # res = self.relu(self.conv4_2(res))
        # out = self.pooling(res)
        
        return out
    
class Conv_Emb_Block(nn.Module):
    def __init__(self, input_dim, output_dim) -> None:
        super(Conv_Emb_Block, self).__init__()
...
train_dataset = AcronymAndSDFDataset(grasp_data, data_dir=data_dir)
...
model = Grasp_Diffusion(device, time_emb=64)
model.to(device)
print()

from torchinfo import summary

input_size1 = torch.Size([32, 1, 32, 32, 32])
...
for epoch in range(10000):
    ...
    for step, (Septernion, sdf, sdf_scale) in enumerate(tqdm(train_dataloader)):
        ...
        # print(noisy.shape, noise.shape, timesteps.shape)
        noisy = noisy.to(device)

        # print(sdf.shape, noisy.shape, timesteps.shape)
        ...
        # writer_v4.add_scalar('Loss/train', loss.detach().item(), epoch)
        ...
        if (global_step % (100_000 //batch_size_num) == 0 and global_step > 0):
            PATH = os.path.join("/home/username/tum-adlr-03-main/training/checkpoints_scale_jc/v2", "trained_model_w_scale.pt")
            torch.save({
                'step': global_step,
            ...
            Loss_step_PATH = os.path.join("/home/username/tum-adlr-03-main/training/checkpoints_scale_jc/v2", "loss_step.json")
