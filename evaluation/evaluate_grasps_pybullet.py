...
import numpy as np
import sys
sys.path.append('/Users/username/Desktop/TUM/Courses/Sem 3/DL_Robotics/Project/tum-adlr-03')
...
__name__ = "grasping-diffusion"

PROJECT_ROOT = __name__
...
"""
the following config part could be deleted? !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
"""

#GLOBAL VARIABLES  - from config file
config = ConfigParser()
print(os.path.join("/Users/username/Desktop/TUM/Courses/Sem 3/DL_Robotics/Project/tum-adlr-03", 'cr_grasper', 'bh_config.ini'))
config.read(os.path.join("/Users/username/Desktop/TUM/Courses/Sem 3/DL_Robotics/Project/tum-adlr-03", 'cr_grasper', 'bh_config.ini'))
...
with open("/Users/username/Desktop/TUM/Courses/Sem 3/DL_Robotics/Project/tum-adlr-03/data/unconstrained_data_split.json") as f:
    val_grasp_files = json.load(f)['val']

with open("/Users/username/Desktop/TUM/Courses/Sem 3/DL_Robotics/Project/tum-adlr-03/models/unconstrained_val_generated_grasps.json") as f:
    generated_grasps = json.load(f)

val_grasp_file = val_grasp_files[1]
# for val_grasp_file in val_grasp_files[0:]:
p.resetSimulation()
success_counter = 0
record_dict = {'success_counter': 0, 'success_index': []}
grasp_dir = "/Users/username/Desktop/TUM/Courses/Sem 3/DL_Robotics/Project/acronym-full-dataset/grasps"
grasp_path = os.path.join(grasp_dir, val_grasp_file)
data = h5py.File(grasp_path, "r")
mesh_fname = data["object/file"][()].decode('utf-8')
mesh_scale = data["object/scale"][()]
mesh_mass = data["/object/mass"][()]
print(mesh_scale)

mesh_root = "/Users/username/Desktop/TUM/Courses/Sem 3/DL_Robotics/Project/acronym-full-dataset"
mesh_path = os.path.join(mesh_root, mesh_fname)

mesh = trimesh.load(mesh_path, force="mesh")
mesh_translated = recenter_(mesh)
mesh_translated.export(os.path.join(os.path.dirname(mesh_path), 'recentered_' + os.path.basename(mesh_fname)))
mesh = mesh.apply_scale(mesh_scale)

if generated_grasps.get(os.path.basename(grasp_path)) is None:
#     continue
    print("NOPE")
g = np.array(generated_grasps[os.path.basename(grasp_path)])

t = [create_gripper_marker(color=[0, 255, 0]).apply_transform(i) for i in g]

# trimesh.Scene([mesh_translated] + t).show()

# p.connect(p.DIRECT)
input_model = os.path.join(os.path.dirname(mesh_path), 'recentered_' + os.path.basename(mesh_fname))
output_model = os.path.join(os.path.dirname(mesh_path), 'p_rendered_' + os.path.basename(mesh_fname))
...
# VHACD parameters can be adjusted based on your needs
    p.vhacd(input_model, output_model,
            output_model.split('.')[0] + '.txt')

mesh_render = trimesh.load_mesh(output_model)
bounds = mesh_render.bounds
mesh_render.show()
# mesh_render = recenter_(mesh_render)
# bounds = mesh_render.bounds
# mesh_render.show()
# mesh_render.export(output_model)
# mesh_render = trimesh.load_mesh(output_model)
# mesh_render.show()
# bounds = mesh_render.bounds
...
def set_ob():
    ...
    # mesh_test = trimesh.load_mesh(output_model)
    # mesh_test.show()
    collisionShapeId = p.createCollisionShape(shapeType=p.GEOM_MESH,
                                                fileName=output_model,
                                                meshScale=[mesh_scale, mesh_scale, mesh_scale])
    # Create the multi-body object using the collision shape
    objectUniqueId = p.createMultiBody(baseMass=mesh_mass,  # Set mass to 0 for static objects
...
gravity_force = [0 * -dynamicsInfo[0] * 9.8, 0 * -dynamicsInfo[0] * 9.8, -1 * dynamicsInfo[0] * 9.8]
max_grasp_force = 6 * dynamicsInfo[0] * 9.8
target_grasp_velocity = 0.3

for i, t_grasp in enumerate(g):
    print(f"grasp:{i}!")
    successful_grasp = [
    ...
    p.changeDynamics(oID, linkIndex=-1, linearDamping=linearDamping, angularDamping=angularDamping)
    dynamicsInfo1 = p.getDynamicsInfo(oID, -1)
    relax(rID)  # open the gripper

    # trimesh.Scene([mesh] + [successful_grasp]).show()
    # check if the pre-grasp position is correct
    # # Create a collision manager instance
    manager = trimesh.collision.CollisionManager()

    # Add the meshes to the collision manager
    manager.add_object("object", mesh)
    manager.add_object("gripper", successful_grasp[0])

    # Check for collisions
    if manager.in_collision_internal():
        continue

    # intersection = mesh.intersection(successful_grasp[0], engine='scad')
    # if not intersection.is_empty:
    #     continue

    p.setPhysicsEngineParameter(contactBreakingThreshold=0.0000000001)
    contact = p.getContactPoints(oID, rID)
    if len(contact) > 0:
        contact_num = 0
        for contact_point in contact:
            # # Each contact point is a dictionary
            # contact_position = contact_point[7]
            # contact_normal = contact_point[9]
            #
            # # Visualize the contact point
            # p.addUserDebugLine(contact_position,
            #                    [contact_position[0] + contact_normal[0] * 0.1,
            #                     contact_position[1] + contact_normal[1] * 0.1,
            #                     contact_position[2] + contact_normal[2] * 0.1],
            #                    lineColorRGB=[1, 0, 0], lineWidth=2.5, lifeTime=1)
            if contact_point[8] < - 0.0001:
                contact_num += 1
                print('wrong!')
        print(f'contact_num: {contact_num}')
    # trimesh.Scene([mesh_translated] + successful_grasp).show()
    ...
    while time() < finish_time:
        ...
        norm_position = [new_position[0] / np.sqrt(new_position[0] ** 2 + new_position[1] ** 2 + new_position[2] ** 2),
                            new_position[1] / np.sqrt(new_position[0] ** 2 + new_position[1] ** 2 + new_position[2] ** 2),
                            new_position[2] / np.sqrt(new_position[0] ** 2 + new_position[1] ** 2 + new_position[2] ** 2)]
        gravity_force = [-norm_position[0] * dynamicsInfo[0] * 9.8,
                            -norm_position[1] * dynamicsInfo[0] * 9.8,
                            -norm_position[2] * dynamicsInfo[0] * 9.8]
    ...
    if len(contact) > 0:
        ...
        p.addUserDebugText("Grav Check Passed!", [-.07, .07, .07], textColorRGB=[0, 1, 0], textSize=1)
        print("Grav Check Passed")
        record_dict['success_counter'] += 1
        record_dict['success_index'].append(i)
        sleep(1)
with open(os.path.join(r'G:\Course_Document\MuR_RCI\23ws\ADLR\acronym\tum_adlr\save', os.path.basename(val_grasp_file) + '.json'), 'w') as file:
